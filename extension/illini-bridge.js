(() => {
  ...
  const isIlliniPage = typeof window !== 'undefined' && ['chocolatte-tracy.github.io', 'illini-day-planner.xdfvvb7ymf.chatgpt.site'].includes(window.location?.hostname);
  const isWorker = typeof window === 'undefined' && !!extensionApi?.runtime?.onMessage;
  ...
  if (isIlliniPage) {
    window.addEventListener('message', async event => {
      ...
      try {
        const result = await extensionApi.runtime.sendMessage({ type: 'bridge-canvas-sync', code: message.code, requestId: message.requestId });
